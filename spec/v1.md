...
- **Storage and retention.** Ledger size caps, eviction, what is written to disk, what is
  logged.
- **The KeyState ratchet.** A door keeps the newest KeyState it accepted for each root DID
  and supplies that record as `pinned` when resolving a later delegated signature. A valid
  higher epoch replaces the pin only after its message passes every refusing rung; an inline
  state older than the pin is refused with the ordinary `-32001` envelope. This makes a
  returning root DID follow a verified op-key rotation and prevents a retired op-key from
  replaying an older, still genuinely root-signed state. The default in-memory store and the
  reference file store implement `getKeyState(rootDid)` / `putKeyState(rootDid, keyState)`;
  an external store implements those beside replay, account, and device-owner state.
...
**The interoperability evidence — two implementations, one shared layer.** The requirements in
§4 are met today by two implementations written independently in different languages, held to
identical verdicts by an acceptance suite that posts identical bytes to both — down to the HTTP
framing — and requires the same status, the same account outcome and the same signed reply from
each. The two share exactly one thing, and it is the part that must not differ: the **wire
layer** — canonical JSON, `did:key`, the six signed fields, the card envelope, the device
binding, the Web Bot Auth verify side — published on its own as
[agent-seam](https://github.com/muretai/agent-seam) (MIT), in both languages, beside the golden
vectors. Everything that *decides* anything — the ladder of §4.5, the account rules, the
transport — is still written twice and shares nothing, and that, rather than a shared library,
is what makes the behavioural requirements here credible: a shared library would have covered
only the parts the two happen to have in common, which is precisely the wire layer and not the
door. If you write a third implementation, that suite is the gate, and we will run it against
yours on request.

The JavaScript implementation is this repository, which carries the seam as a pinned copy
(`vendor/agent-seam/VENDOR.json`). The Python one lives in Muretai core, which vendors this
package at a pinned version and runs that suite against both. Neither repository owns the
other's implementation, and the vectors both are held to are agent-seam's.
